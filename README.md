...
The Countries project is responsible for listing countries and their details, including a chart that shows the country’s population over time.
  ...
  - :warning: Before starting, your docker-compose needs to be version 25 or higher (preferably 27.2).
  
  - :information_source: Upload the complete project, including your environment variables (if any), using the command docker-compose up --build at the root of the project.
  
  - These services will initialize containers named app_frontend, backend_app, and app_mongo.

  - From here, you can run the containers via CLI or open them in VS Code.

  - :information_source: Dependencies are installed through the Dockerfile, which is read by Docker.
  
  - :warning: Attention: The git inside the container is not configured with your credentials. Either make commits outside the container or configure your git credentials inside the container.
  - :warning: Attention: Do not run the command npm audit fix! It updates several project dependencies that can cause conflicts.

  ✨ Tip: The Remote - Containers extension is recommended so you can develop your application in the Docker container directly in VS Code, just like you do with your local files.
